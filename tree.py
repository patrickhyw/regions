...
# root. Generated by Opus 4.6. See replace.json.
_REPLACE_PATH = Path(__file__).parent / "replace.json"
...
class TreeSpec:
    ...
    Defaults are the minimalist setup (num_lemmas=1,
    include_definition=False) with ANIMAL_REPLACE.
    """

    root_synset: str
    dag_handling: Literal["disallow", "collapse"] = "disallow"
    dup_handling: Literal["disallow", "prune"] = "disallow"
    num_lemmas: int | Literal["all"] = 1
    include_definition: bool = False
    replace: dict[str, str | None] = field(default_factory=lambda: ANIMAL_REPLACE)
